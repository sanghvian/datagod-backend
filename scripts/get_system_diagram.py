...
def return_infra_code(prompt):
    ...
    result= chain.run({'text': prompt, 'content': 'Strictly return only the Python code in string format and no other extra string data'})
    return result
...
def upload_image_to_s3(buffer: BytesIO, key: str, bucket_name: str, image_format: str = "png") -> str:
    s3 = boto3.client('s3')
    s3.upload_fileobj(buffer, bucket_name, key, ExtraArgs={"ContentType": f"image/{image_format}"})
